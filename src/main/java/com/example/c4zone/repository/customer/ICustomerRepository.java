...
public interface ICustomerRepository extends JpaRepository<Customer, Long> {
   ...
   void saveCustomer(Customer customer);
    ...
    @Query(value = "select customer.*, count(order_bill.id_customer ) as total_purchases from customer left join order_bill on customer.id_customer = order_bill.id_customer where name_customer like :name and TIMESTAMPDIFF(YEAR, birth_date_customer, CURDATE()) >= 0 group by customer.id_customer ", nativeQuery = true)
