...
public interface SalesReportRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "WITH RECURSIVE DateRange AS ("
            ...
            + "dr.`date` AS `date`, "
            + "'Tất Cả' AS `name`, "
            + "COALESCE(SUM(((od.price_order - p.price_product) * od.quantity_order)*0.9), 0) AS `daily`, "
    ...
    @Query(value = "SELECT COALESCE(SUM((od.price_order)* od.quantity_order), 0) AS daily_revenue" +
    ...
    @Query(value = "SELECT COALESCE(SUM((od.price_order)* od.quantity_order), 0) AS monthly_revenue" +
            " FROM order_detail od" +
            " INNER JOIN product p ON od.id_product = p.id_product" +
            " INNER JOIN order_bill ob ON od.id_order = ob.id_order_bill" +
    ...
    @Query(value = "SELECT t.id_type AS typeid,t.name AS typename,COALESCE(SUM((od.price_order)* od.quantity_order), 0) AS revenue FROM order_bill ob JOIN order_detail od ON ob.id_order_bill = od.id_order JOIN product p ON od.id_product = p.id_product JOIN type t ON p.id_type = t.id_type WHERE MONTH(ob.date_of_order) = MONTH(CURRENT_DATE)GROUP BY t.id_type, t.name;\n",nativeQuery = true)
